...
void PATEventTree::beginJob() {
  ...
  fTree->Branch("HLTP_IsoMu24_Filters", fHLTP_IsoMu24_Filters, "HLTP_IsoMu24_Filters[7]/O");
  ...
  fTree->Branch("HLT_Muon_VertexmumuJpsi",       fHLT_Muon_VertexmumuJpsi,          "HLT_Muon_VertexmumuJpsi[HLTN]/F");
   ...
   //For Z matching
   fTree->Branch("HLT_MuonZ_L3Matching_Pt",  fHLT_MuonZ_L3Matching_Pt,          "HLT_MuonZ_L3Matching_Pt[10]/F");
   fTree->Branch("HLT_MuonZ_L3Matching_Eta", fHLT_MuonZ_L3Matching_Eta,          "HLT_MuonZ_L3Matching_Eta[10]/F");
   fTree->Branch("HLT_MuonZ_L3Matching_Phi", fHLT_MuonZ_L3Matching_Phi,          "HLT_MuonZ_L3Matching_Phi[10]/F");
   fTree->Branch("HLT_MuonZ_VtxMatching_Pt",  fHLT_MuonZ_VtxMatching_Pt,          "HLT_MuonZ_VtxMatchingZ_Pt[10]/F");
   fTree->Branch("HLT_MuonZ_VtxMatching_Eta", fHLT_MuonZ_VtxMatching_Eta,          "HLT_MuonZ_VtxMatchingZ_Eta[10]/F");
   fTree->Branch("HLT_MuonZ_VtxMatching_Phi", fHLT_MuonZ_VtxMatching_Phi,          "HLT_MuonZ_VtxMatchingZ_Phi[10]/F");
   //end
   fTree->Branch("HLT_Muon_VertexmumuJpsi",       fHLT_Muon_VertexmumuJpsi,          "HLT_Muon_VertexmumuJpsi[HLTN]/F");
   fTree->Branch("HLT_Muon_TripleMuL3",         fHLT_Muon_TripleMuL3,          "HLT_Muon_TripleMuL3[HLTN]/F");
   //end                                                                                                                                                      
...
*/
  ...
  if ( fHLT_Skim && !( fHLTP_Trimuon5_3p5_2_Upsilon_Muon[1] || fHLTP_Dimuon0_Jpsi3p5_Muon2[1] || fHLTP_IsoMu24[1] || fHLTP_IsoMu27[1] || fHLTP_Mu50[1])) return; 
...
void PATEventTree::init() {
  ...
  fHLTP_DoubleMu3_PS = fHLTP_DoubleMu6_PS = fHLTP_DoubleMu7_PS = fHLTP_Dimuon0_Upsilon_Muon_PS = fHLTP_Dimuon0_Jpsi3p5_Muon2_PS = fHLTP_Dimuon0_Jpsi_PS = fHLTP_Dimuon10_Jpsi_Barrel_PS = fHLTP_TripleMu5_PS = fHLTP_QuadMuon0_Dimuon0_Jpsi_PS = fHLTP_QuadMuon0_Dimuon0_Upsilon_PS = fHLTP_Trimuon5_3p5_2_Upsilon_Muon_PS = fHLTP_Trimuon2_Upsilon5_Muon_PS = fHLTP_TrimuonOpen_5_3p5_2_Upsilon_Muon_PS =fHLTP_IsoMu24_PS=fHLTP_IsoMu27_PS = fHLTP_Mu50_PS= 0;

  for (int i = 0; i < 3; i++) {
    fHLTP_DoubleMu3[i] = fHLTP_DoubleMu6[i] = fHLTP_DoubleMu7[i] = fHLTP_Dimuon0_Upsilon_Muon[i] = fHLTP_Dimuon0_Jpsi3p5_Muon2[i] = fHLTP_Dimuon0_Jpsi[i] = fHLTP_Dimuon10_Jpsi_Barrel[i] = fHLTP_TripleMu5[i] = fHLTP_QuadMuon0_Dimuon0_Jpsi[i] = fHLTP_QuadMuon0_Dimuon0_Upsilon[i] = fHLTP_Trimuon5_3p5_2_Upsilon_Muon[i] = fHLTP_Trimuon2_Upsilon5_Muon[i] = fHLTP_TrimuonOpen_5_3p5_2_Upsilon_Muon[i] = fHLTP_IsoMu24[i]= fHLTP_IsoMu27[i] = fHLTP_Mu50[i] = false;
  }
  for (int i=0; i<11; i++){
    fHLTP_Trimuon5_3p5_2_Upsilon_Muon_Filters[i]=false;
  }
  for (int i = 0; i < 5; i++) {
    fHLTP_DoubleMu3_Filters[i] = fHLTP_DoubleMu6_Filters[i] = fHLTP_DoubleMu7_Filters[i] = fHLTP_TripleMu5_Filters[i] = fHLTP_Dimuon0_Jpsi_Filters[i] = fHLTP_Dimuon0_Jpsi3p5_Muon2_Filters[i] = fHLTP_Dimuon10_Jpsi_Barrel_Filters[i] = fHLTP_IsoMu24_Filters[i]=fHLTP_IsoMu27_Filters[i] = fHLTP_Mu50_Filters[i] = false;
  }
  fHLTP_IsoMu24_Filters[5]=fHLTP_IsoMu24_Filters[6]=fHLTP_IsoMu27_Filters[5]=fHLTP_IsoMu27_Filters[6]=fHLTP_Mu50_Filters[5]=fHLTP_Dimuon0_Jpsi_Filters[5] = fHLTP_Dimuon0_Jpsi_Filters[6] = fHLTP_Dimuon0_Jpsi3p5_Muon2_Filters[5] = fHLTP_Dimuon0_Jpsi3p5_Muon2_Filters[6] = fHLTP_Dimuon0_Jpsi3p5_Muon2_Filters[7] = fHLTP_Dimuon10_Jpsi_Barrel_Filters[5] = fHLTP_Dimuon10_Jpsi_Barrel_Filters[6] = false;
...
void PATEventTree::fillHLTPath(const unsigned int psSet, const edm::Handle<edm::TriggerResults>& hHLTresults, const trigger::TriggerEvent& triggerEvent) {
  ...
  for (unsigned int itrig = 0; itrig < validTriggerNames.size(); itrig++) {
    ...
    } else if (validTriggerNames[itrig].find("HLT_IsoMu24_v")!=string::npos) {
  ...
  for(uint filterIndex=0;filterIndex<triggerEvent.sizeFilters();++filterIndex){  //loop over all trigger filters in event (i.e. filters passed)
    ...
    } else if(label.find( "ltL1fL1sMu22L1Filtered0" ) != string::npos ) {
      fHLTP_IsoMu24_Filters[2]=true;
    } else if(label.find( "hltL3fL1sSingleMu22L1f0L2f10QL3Filtered24Q" ) != string::npos ) {
      fHLTP_IsoMu24_Filters[3]=true;
    } else if(label.find( "HLTMu24IsolationSequence") != string::npos ) {
      fHLTP_IsoMu24_Filters[4]=true;
    } else if(label.find( "hltL3crIsoL1sSingleMu22L1f0L2f10QL3f24QL3trkIsoFiltered0p07" ) != string::npos ) {
      fHLTP_IsoMu24_Filters[5]=true;
    } else if(label.find( "hltL1fForIterL3L1fL1sMu22L1Filtered0" ) != string::npos ) {
      fHLTP_IsoMu24_Filters[6]=true;
    ...
    } else if(label.find( "HLTMu27IsolationSequence" ) != string::npos ) {
...
void PATEventTree::fillHLT(const pat::TriggerObjectStandAloneCollection& HLTObjects) {
  ...
  std::string filterZ1 = "hltL3crIsoL1sSingleMu22L1f0L2f10QL3f24QL3trkIsoFiltered0p07"; //L3 Trigger Path for Z trigger Iso24
  std::string filterZ  = "hltL3crIsoL1sMu22Or25L1f0L2f10QL3f27QL3trkIsoFiltered0p07"; //L3 Trigger Path for Z trigger Iso27
...
void PATEventTree::fillJPsiMuMuCand(const reco::CompositeCandidateCollection& JPsiCands, const reco::MuonRefVector& goodMuons, std::vector<TransientTrack>& t_tks) {
  ...
  for(unsigned int ijp = 0; ijp < JPsiCands.size(); ijp++) {
    ...
    if( fJPsiMuCategory[fJPsiN][0]>0 && fJPsiMuCategory[fJPsiN][1]>0 && fJPsiMass[0]>2.8 && fJPsiMass[0]<110.0 ) {//changed for Upsilon
...
void PATEventTree::makeEtabCand(std::vector<TransientTrack>& t_tks) {
  ...
  for(int ijp1 = 0; ijp1 < fJPsiMuMuN-1; ijp1++) {
    ...
    for(int ijp2 = ijp1+1; ijp2 < fJPsiMuMuN; ijp2++) {
      ...
      if (fEtabProb[fEtabN]<0.005) continue;
